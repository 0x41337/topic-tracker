...
import { computeMove, newId, nextName, normalizeName, subtreeIds } from "./tree"

/* ---------- Observable store (framework-free) ----------
 *
 * Holds the tree + sessions, persists to localStorage and notifies
 * subscribers. React bindings live in store-react.ts; swap that file (or use
 * the store directly) to drive the state from another UI layer.
 */
