...
import { formatPercent, formatShortDate, topicStats } from "@/lib/tree-utils"
...
export function PracticeView({ topicId }: { topicId: string }) {
    ...
    const node = ctx.state.nodes[topicId]
    const today = new Date().toISOString().slice(0, 10)
    const session = (ctx.state.sessions[topicId] ?? []).find(
        (s) => s.date === today,
    )
    const hits = session?.hits ?? 0
    const total = session?.total ?? 0
    const overall = topicStats(ctx.state.sessions[topicId])
